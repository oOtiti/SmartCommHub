...
const SystemSettings = () => {
  ...
  const handleCreateElderOrFamily = async (values: { username: string; password: string; phone?: string; user_type: UserTypeOption; elderly_id?: number; family_id?: number; }) => {
    try {
  ...
  const handleCreateProvider = async (values: { username: string; password: string; phone?: string; provider_id?: number; }) => {
    try {
  ...
  if (!isAdmin) {
    return <RequireAuth><div style={{ padding: 24 }}>正在验证权限...</div></RequireAuth>;
  }
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card title="创建老人/家庭成员" bordered>
          <Form form={formElderFamily} layout="vertical" onFinish={handleCreateElderOrFamily}>
            <Form.Item name="user_type" label="类型" rules={[{ required: true, message: '请选择类型' }]}> 
              <Select options={[{ value: 1, label: '老人(1)' }, { value: 2, label: '家庭成员(2)' }]} />
            </Form.Item>
        ...
        <Card title="管理员重置任意账户密码" bordered>
          <Form form={formResetPwd} layout="vertical" onFinish={handleAdminResetPassword}>
            <Form.Item name="username" label="目标用户名" rules={[{ required: true, message: '请输入目标用户名' }]}> 
